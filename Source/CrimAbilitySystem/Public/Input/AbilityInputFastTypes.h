...
struct CRIMABILITYSYSTEM_API FAbilityInputContainer : public FFastArraySerializer
{
	...
	/** Removes all instances with matching AbilityClass. Returns the slots that were removed. */
	TArray<FAbilityInputSlot> RemoveAbilityInputInstance(const TSubclassOf<UGameplayAbility>& AbilityClass);
